...
export default function App() {
    ...
    const fetchSavedBlogs = async () => {
        try {
            const response = await fetch('https://api.blog.omnicassion.com/saved-blogs');
    ...
    const fetchScheduledBlogs = async () => {
        try {
            const res = await fetch('https://api.blog.omnicassion.com/scheduled-blogs');
    ...
    const handleSaveBlog = async (blog, content, titleImage) => {
        try {
            const response = await fetch(`https://api.blog.omnicassion.com/update-blog/${blog._id}`, {
            ...
            fetchSavedBlogs();
            fetchScheduledBlogs();
            setView({ name: blog.source === 'saved' ? 'saved' : 'schedule', data: null });
        } catch (err) { setError(err.message); }
    };

    const handleSaveToDB = async (blogId) => {
        try {
            const response = await fetch(`https://api.blog.omnicassion.com/save-blog/${blogId}`, { method: 'POST' });
    ...
    const handleDeleteBlog = async (blogId) => {
        try {
            const response = await fetch(`https://api.blog.omnicassion.com/blogs/${blogId}`, { method: 'DELETE' });
    ...
    const handleSchedule = async (topic, scheduledTime, titleImage, category = 'general', priority = 'medium', content = null) => {
        try {
            const response = await fetch('https://api.blog.omnicassion.com/schedule-blog', {
    ...
    const handleRefineSubmit = async () => {
        ...
        try {
            ...
            const response = await fetch('https://api.blog.omnicassion.com/generate-blog', {
                method: 'POST', headers: { 'Content-Type': 'application/json' },
    ...
    const handleSaveFromChat = async (blogContent) => {
        try {
            const response = await fetch('https://api.blog.omnicassion.com/save-generated-blog', {
                method: 'POST',
    ...
    const handleClearScheduled = async () => {
        ...
        try {
            const deletePromises = scheduledBlogs.map(blog => fetch(`https://api.blog.omnicassion.com/blogs/${blog._id}`, { method: 'DELETE' }));
    ...
    const renderView = () => {
        if (view.name === 'editor') {
            ...
            return <BlogEditor
                blog={fullBlogData}
                onBack={() => setView(view.data.source === 'saved' ? { name: 'saved' } : { name: 'schedule' })}
        ...
        switch (view.name) {
            case 'chat':
                return <ChatView isLoading={isLoading} chatLog={chatLog} question={question} setQuestion={setQuestion} handleKeyDown={handleKeyDown} askQuestion={askQuestion} chatEndRef={chatEndRef} handleSaveBlog={handleSaveFromChat} />;
            case 'schedule':
                return <ScheduleView filteredScheduledBlogs={filteredScheduledBlogs} setSearchTerm={setSearchTerm} setFilterStatus={setFilterStatus} filterStatus={filterStatus} setIsModalOpen={setIsModalOpen} handleClearScheduled={handleClearScheduled} setView={setView} handleSaveToDB={handleSaveToDB} handleDeleteBlog={handleDeleteBlog} setSchedulingMode={setSchedulingMode} />;
            // --- NEW VIEW CASE ---
            case 'saved':
                return <SavedBlogsView
